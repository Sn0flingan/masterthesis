function [ score ] = sepScoreRobust_norm_noglob( X, idx_classes)
...
for i=1:length(idx_classes)
    ...
    %Get distance to all other classes
    for j=1:length(idx_classes)
        if(~(i==j))
            mClass(j,:) = median(X(:,idx_classes{j})');
            b_all(j) = norm(mClass(i,:)-mClass(j,:),1)/norm(mClass(i,:),1);
        else
            b_all(j) = nan; %ignore distance to same class
        end
    end
    b = b + nanmean(b_all);
end
