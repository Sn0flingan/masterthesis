...
dataSamp = cleanData(dataSamp, dispFlag);
...
for i=0:50
    for j = 1:10
        ...
        %instead
        try
            Ibatch = batchRmOSSLDA(dataSamp, idxTissues, dBatch);
        ...
        try
            I = dataBatch.I;
            % Calculate score
            scoreSepT(j) = sepScoreRobust_norm(I, idxOrigins);
            disp(['Seperation score is ' num2str(scoreSepT(j))]);
        catch ME
    ...
    scoreSep(i+1) = mean(scoreSepT);
    scoreStd(i+1) = std(scoreSepT);
end
toc
