function [ Iclean ] = blankSubtraction_2( Isamples, Iblanks, dispFlag)
...
cleanedFeat = [];
Iclean = zeros(size(Isamples,1),size(Isamples,2)); %Initally set all signals to 0 as baseline
%Correct blank background in each feature
for i=1:size(Iblanks,1)
    %Identify if the signal is just blanks or blanks + biological
    %signal(outliers) with 5 sigma
    outliers = [];
    outliers = find(abs(median(Iblanks(i,:)')-Isamples(i,:))>medianStd(Iblanks(i,:)')*5);
    Iclean(i,outliers) = Isamples(i,outliers) - median(Iblanks(i,:)')'*ones(1,length(outliers));
    if(~isempty(outliers))
        disp([num2str(length(outliers))])
        cleanedFeat = [cleanedFeat; i];
    end
end
...
%Report results
if(dispFlag>0)
    cleanedFeat = unique(cleanedFeat);
    disp(['---'])
    disp(['Blank Subtraction finnished.'])
    disp(['Time to complete: ', num2str(toc_bs) 's .'])
    disp(['Number of features cleaned: ' num2str(numel(cleanedFeat))...
        '(' num2str(numel(cleanedFeat)/size(Iclean,1)*100) '%)']);
    disp(['---'])
end
