function [ I ] = cyclicLoess(I, minA, smoothingParam, dispFlag )
...
span = 0.7; %same default as Ballman article

meanI = median(I')'; %mean sample
%For each sample
for j=1:size(I,2)
    clear fit1 f
    A = meanI;
    ...
    xdata = A(idx);
    ydata = M(idx);
    yTrend = smooth(xdata, ydata, span, 'rloess');
    I(idx,j) = I(idx,j) - yTrend; %correct data
    if(mod(j,1)==0 && dispFlag>1)
...
if(dispFlag>0)
    ...
    if(dispFlag>1)
        ...
        imagesc(Ipre, cLim)
        colorbar()
        title('Intensities before normalization')
        subplot(2,2,2)
        imagesc(I, cLim)
        colorbar()
        title('Intensities after normalization')
        subplot(2,2,3)
        hold on
        for i=1:size(Ipre,2)
            histogram(Ipre(:,i))
        ...
        hold off
        title('Distributions after normalization')
    end
end
